...
APP="acme-dns"
var_tags="${var_tags:-dns}"                     # Max 2 tags, semicolon-separated
...
function update_script() {
  ...
  if check_for_gh_release "acme-dns" "acme-dns/acme-dns"; then
...
#    cp -r /opt/acme-dns/data /opt/acme-dns_data_backup 2>/dev/null || true
#    msg_ok "Backed up Data"

    # Step 5: Download and deploy new version
    # CLEAN_INSTALL=1 removes old directory before extracting
    CLEAN_INSTALL=1 fetch_and_deploy_gh_release "acme-dns" "owner/repo" "tarball" "latest" "/opt/acme-dns"
...
#    cp -r /opt/acme-dns_data_backup/. /opt/acme-dns/data/ 2>/dev/null || true
#    rm -rf /opt/acme-dns_data_backup
