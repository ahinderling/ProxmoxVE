...
fetch_and_deploy_gh_release "acme-dns" "acme-dns/acme-dns" "tarball" "latest" "/opt/acme-dns"
...
# create_self_signed_cert                  # Creates cert in /etc/ssl/acme-dns/
...
msg_info "Setting up acme-dns"
$STD adduser --system --gecos "acme-dns Service" --disabled-password --group --home /var/lib/acme-dns acme-dns
$STD mkdir /var/lib/acme-dns /etc/acme-dns
$STD chown -R acme-dns:acme-dns /var/lib/acme-dns /etc/acme-dns /opt/acme-dns
cd /opt/acme-dns
$STD install -m 755 acme-dns /usr/local/bin

msg_ok "Setup acme-dns"

# =============================================================================
# CONFIGURATION
# =============================================================================

msg_info "Configuring acme-dns"
cd /opt/acme-dns
$STD install -m 644 config.cfg /etc/acme-dns
...
cd /opt/acme-dns
$STD install -m 644 acme-dns.service /etc/systemd/system

#cat <<EOF >/etc/systemd/system/acme-dns.service
#[Unit]
...
#ExecStart=/usr/bin/node /opt/acme-dns/server.js
#Restart=on-failure
#RestartSec=5
#
#[Install]
...
systemctl enable -q --now acme-dns
